...
export default function userReducer(state = Immutable({}), action) {
    ...
    switch (action.type) {
        ...
        case ADD_PAPER_ACTION_TYPE: {
            ...
            const newCategories = [...state.categories.filter(c => c._id !== action.payload.categoryId), category];
        ...
        case ADD_SEARCH_ACTION_TYPE:
            newState = Immutable({...state, searches: [...state.searches, action.payload.search]});
            break;
